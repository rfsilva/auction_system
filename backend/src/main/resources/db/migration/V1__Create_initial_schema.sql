-- =====================================================
-- Sistema de Leilão Eletrônico - Schema Consolidado
-- Versão: 1.0 - Base de Desenvolvimento Limpa
-- Data: 2024-12-19
-- Descrição: Schema completo consolidado com todas as estruturas, índices e usuário root
-- =====================================================
...
CREATE TABLE tb_contrato (
    ...
    FOREIGN KEY (created_by) REFERENCES tb_usuario(id),
...
CREATE TABLE tb_lote (
    ...
    FOREIGN KEY (contract_id) REFERENCES tb_contrato(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = 'Tabela de lotes - vinculada a contratos';
...
-- Índices compostos para consultas de contratos ativos por vendedor e período
CREATE INDEX idx_tb_contrato_active_seller_period ON tb_contrato(active, seller_id, valid_from, valid_to);

-- Índice para consultas de contratos vencendo
...
-- Usuário administrador padrão (root)
...
1. TODAS AS TABELAS:
   ...
   - Documentos e disputas

2. ÍNDICES COMPLETOS:
   - Índices básicos para todas as tabelas
   - Índices compostos para performance
   - Índices específicos para consultas complexas
...
PRÓXIMOS PASSOS:
- V2: Massa de dados para desenvolvimento
